...
* hanke: (server) BREAKING Renamed the undocumented @Search#maximum_tokens(integer)@ to @Search#max_words(integer)@. Restricts the amoun
...
* hanke: (server) Added @<<@ and @unshift@ on @Index@ and @Category@. The @unshift@ method behaves like the @add@ method when called without a second parameter. Use like @index << Thing.new(1, 'some text', 'some other text')@.
