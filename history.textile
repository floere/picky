...
* hanke: (server) Breaking: Index::Memory and Index::Redis do not accept options anymore. Define options in the block or on the resulting instances.
* hanke: (server) Breaking: PickyLog removed. In the classic server, use Picky.logger = MyLogger.new to log (uses #info). In the Sinatra server, use MyLogger.info result.to_log(params[:query]) if you want to log.
