...
h(p,v);if(x!=undefined)p=x;s=[c,p,f,v];x=p;if(x!=z()){x="?q="+escape(x).replace(/\*/g,"%2A");window.History&&window.History.getState()&&window.History.pushState&&window.History.pushState(null,null,x)}if(p=="")b.reset();else(c=k[c])&&c.search(p,f,v)};this.resend=function(){s&&j.apply(this,s)};var e=function(c,p){c=m(c,p)||c;b.fullResultsCallback(c);o(c,p)},d=function(c,p){clearInterval(i);j("full",c,e,p||{})};a=function(c,p){c=m(c,p)||c;b.liveResultsCallback(c);o(c,p)};var n=u?e:a,i,q=function(){var c=
